# Event Stroom-Dashboard — roadmap-overzicht

> Voor omschrijving en featurelijst: zie [event_dashboard.md](event_dashboard.md). De roadmap zelf
> staat per versie in een eigen bestand, gesplitst omdat één gecombineerd bestand te groot werd.
> Houd het juiste bestand bij als de roadmap wijzigt, ongeacht wie die wijziging doorvoert.

- [roadmap_v2.md](roadmap_v2.md) — afgerond, uitgebracht als
  [v2.0.0](https://github.com/Tbizla/stroom-dashboard/releases/tag/v2.0.0)
- [roadmap_v3.md](roadmap_v3.md) — afgerond (alle punten `[x]`, "Ideeën van Claude"-sectie leeg).
  Sinds de overstap naar echte semver (CLAUDE.md, §Versionering) geen eigen enkele release-tag zoals
  v2 — de punten zijn incrementeel uitgebracht, van
  [v2.1.0](https://github.com/Tbizla/stroom-dashboard/releases/tag/v2.1.0) (knikpunten, het eerste
  afgeronde punt) t/m [v3.7.0](https://github.com/Tbizla/stroom-dashboard/releases/tag/v3.7.0)
- [roadmap_v4.md](roadmap_v4.md) — actief sinds 18 augustus 2026 (vooralsnog alleen losse tweaks/
  bugfixes tussendoor, `[x]`) — de acht oorspronkelijk geplande punten (met Mike geprioriteerd op
  3 augustus 2026, deels vanuit v3, deels vanuit diens toenmalige "Ideeën van Claude"-sectie) staan
  nog `[ ]`, nog niet gestart.
